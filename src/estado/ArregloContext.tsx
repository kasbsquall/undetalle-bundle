...
import {
  ARTICULOS, FORMATOS, CINTAS, ENVIO_GRATIS_DESDE,
  tramoPara, siguienteTramo,
  type Articulo,
} from '../datos/catalogo'
...
export function ProveedorArreglo({ children }: { children: ReactNode }) {
  ...
  const cambiar = (id: string, delta: number) =>
    setCantidades((prev) => {
      const siguiente = Math.max(0, (prev[id] ?? 0) + delta)
      const copia = { ...prev }
  ...
  const valor = useMemo<Valor>(() => {
    ...
    const tramo = tramoPara(subtotal)
    const siguiente = siguienteTramo(subtotal)
    ...
    return {
      ...
      llevaEnvioGratis: subtotal >= ENVIO_GRATIS_DESDE,
      faltaParaEnvioGratis: +Math.max(0, ENVIO_GRATIS_DESDE - subtotal).toFixed(2),
    }
  }, [formatoId, cintaId, cantidades, mensaje])
